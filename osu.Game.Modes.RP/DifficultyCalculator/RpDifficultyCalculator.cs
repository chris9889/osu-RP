...
namespace osu.Game.Modes.RP.DifficultyCalculator
{
    public class RpDifficultyCalculator : DifficultyCalculator<BaseRpObject>
    {
        ...
        public RpDifficultyCalculator(Beatmap beatmap) : base(beatmap)
        {
        ...
        protected override void PreprocessHitObjects()
        {
            foreach (var h in Objects)
                if (h.ObjectType == ObjectType.LongTail)
                    ((RpLongTailObject)h).Curve.Calculate();
        }
        ...
        protected override double CalculateInternal(Dictionary<String, String> categoryDifficulty)
        {
            ...
            if (categoryDifficulty != null)
            {
                ...
                int maxCombo = 0;
                foreach (RpHitObjectDifficulty hitObject in DifficultyHitObjects)
        ...
        protected bool CalculateStrainValues()
        {
            ...
            List<RpHitObjectDifficulty>.Enumerator hitObjectsEnumerator = DifficultyHitObjects.GetEnumerator();

            if (!hitObjectsEnumerator.MoveNext()) return false;

            RpHitObjectDifficulty currentHitObject = hitObjectsEnumerator.Current;
        ...
        protected double CalculateDifficulty(DifficultyType type)
        {
            ...
            foreach (RpHitObjectDifficulty hitObject in DifficultyHitObjects)
            {
                ...
                while (hitObject.BaseHitObject.StartTime > intervalEndTime)
                {
                    ...
                    {
                        double decay = Math.Pow(RpHitObjectDifficulty.DECAY_BASE[(int)type], (intervalEndTime - previousHitObject.BaseHitObject.StartTime) / 1000);
