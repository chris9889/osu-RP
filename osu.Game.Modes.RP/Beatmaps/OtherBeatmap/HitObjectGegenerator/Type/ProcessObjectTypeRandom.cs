...
namespace osu.Game.Modes.RP.Beatmaps.OtherBeatmap.HitObjectGegenerator.Type
{
    public class ProcessObjectTypeRandom
    {
        ...
        /// </summary>
        /// <returns></returns>
        RpBaseHitObjectType.Shape CalRandomShape(SingleHitObjectConvertParameter singleTuple,int index)
        {
            int randNum = CalRandNumber(singleTuple, index);
            switch (randNum % 4)
            {
                case 0:
        ...
        /// </summary>
        /// <returns></returns>
        int CalRandNumber(SingleHitObjectConvertParameter singleTuple,int index)
        {
            int addNumber = 0;
            if (index == 1) //第二個物件
                addNumber = 2;
