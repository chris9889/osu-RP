...
namespace osu.Game.Modes.RP.UI.GamePlay.Playfield.Layout.HitObject
{
    ...
    internal class HitObjectLayout : BaseGamePlayLayout
    {
        ...
        public void AddDrawableBaseHitObject(DrawableBaseHitObject drawableHitObject)
        {
            try
            {
                ...
                if (drawableHitObject is DrawableRpLongPress)
                    _backgroundLayout.GetContainerByTime(drawableHitObject.HitObject.StartTime).ElementAt(containerIndex).ContainerTemplate.AddObject(drawableHitObject as DrawableRpLongPress);
                else
                    _backgroundLayout.GetContainerByTime(drawableHitObject.HitObject.StartTime).ElementAt(containerIndex).ContainerTemplate.ListLayoutTemplate[layoutIndex].AddObject(drawableHitObject);
            }
            catch
            {
        ...
        {
            var listContainer = _backgroundLayout.GetContainerByTime(time);
