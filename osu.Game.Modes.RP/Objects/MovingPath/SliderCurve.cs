...
namespace osu.Game.Modes.RP.Objects.MovingPath
{
    ...
    public class SliderCurve
    {
        ...
        public void AntoGenerateEndPosition(Vector2 Direction)
        {
            ...
            if (ControlPoints.Count == 1)
            {
                int _canvasWidth = 512*2;
                int _canvasHeight = 384*2;
                //需要加上這個確保物件在外面
                float _multiple = 1.5f;
        ...
        public void GenrtateMiddlePath()
        {
            ...
            {
                Vector2 middlePosition1 = StartPosition - (StartPosition - EndPosition) / 3 * 1 + new Vector2(200, 200);
                Vector2 middlePosition2 = StartPosition - (StartPosition - EndPosition) / 3 * 2 - new Vector2(200, 200);
